...
describe("purge", () => {
    ...
    it("drops old queries", async () => {
        ...
        expect((await store.getQueryList()).length).to.eq(1);
        now = new Date(now.getTime() + 60 * 60 * 1000); // one hour later
        await store.purge();
        expect((await store.getQueryList()).length).to.eq(1); // not dropped
        now = new Date(now.getTime() + 1000); // but one more second later
        await store.purge();
        expect((await store.getQueryList()).length).to.eq(0); // dropped!
    });
    ...
    it("does not drop old and active queries", async () => {
        ...
        expect((await store.getQueryList()).length).to.eq(1); // not dropped (since it was active)
    });
    ...
    it("drops old rejected commands", async () => {
        ...
        expect((await store.getCommandList()).length).to.eq(1);
        now = new Date(now.getTime() + 5 * 60 * 1000); // five minutes later
        await store.purge();
        expect((await store.getCommandList()).length).to.eq(1); // not dropped
        now = new Date(now.getTime() + 1000); // but one more second later
        await store.purge();
        expect((await store.getCommandList()).length).to.eq(0); // dropped!
    });
